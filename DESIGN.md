...
| 작성일 | 2026-08-18 |
| 대상 | `PRD.md` v0.3 구현 |
| 방향 | 글래스모피즘 · 고채도 배경 · Pretendard 단일 서체 · 실사 사진 마커 + Leaflet 세계지도 |
...
### 4.3 랜드마크 사진 핀 (UI 유리와 구분)

지도가 실사 타일로 바뀌면서 랜드마크는 SVG 실루엣이 아니라 **원형 실사 사진 마커**(Leaflet `divIcon`)가 된다. 유리 두 종류를 나누던 원칙은 유지하되, 대상이 "테두리 있는 도형"에서 "테두리 있는 사진 핀"으로 바뀐다 — **핀은 테두리로, UI는 블러로** 존재감을 만든다.

| 속성 | 랜드마크 사진 핀 | UI 패널 |
|---|---|---|
| 블러 | **없음** | 강함 (12–24px) |
| 채움 | `background-image` 사진, `object-fit:cover`, 원형 크롭(`border-radius:50%`) | `rgba(255,255,255,0.14)`, 스크림 있음 |
| 테두리 | **3px `rgba(255,255,255,0.9)`** — 강함, 사진이 배경 위에서 확실히 분리되도록 | 1px `rgba(255,255,255,0.22)` — 약함 |
| 그림자 | `0 2px 8px rgba(0,0,0,0.35)` — 지도 타일 위에서 핀이 "떠 있는" 느낌을 주기 위해 필요 | 있음 |
| 크기 | 기본 44px 지름(터치 타깃 최소값과 동일), hover/focus/선택 시 52px | — |
...
.landmark-pin {
  ...
  transition: width .16s ease-out, height .16s ease-out, border-color .16s ease-out;
}
```
...
|---|---|
| 기본 | 위 스펙 그대로 |
| hover / focus | 44px→52px 확대, 테두리 `#fff` 100%, 라벨 툴팁 표시 |
| focus (키보드) | 추가로 `--accent` 2px 아웃라인(`outline`, 핀 바깥쪽) |
| 선택됨 | 테두리를 `--accent` 3px로 전환, 52px 유지 |
| 필터 제외 | `opacity: 0.2` + `filter: grayscale(1)` (PRD FR-04-1) |
| 사진 로드 실패 | `background-image` 대신 `--glass-l3` 배경 + 랜드마크 대표 이모지(48% 크기)로 폴백 |
| 검색으로 생성된 임시 마커 | 사진 없이 단색 `--accent` 배경 + 📍 이모지, 지름 36px(프리셋 마커보다 작게 해 시각적으로 "임시"임을 구분) |
...
구성: `이모지 14px` + `기온 12px/600`. 사진 핀(44px 원형, §4.3) 상단 중앙에서 8px 위 — 핀 크기가 고정이므로 랜드마크별 높이 보정이 필요 없다.
...
| # | 항목 | 처리 |
|---|---|---|
| 1 | 랜드마크 12곳 실사 사진 URL(Wikimedia Commons) | PRD FR-02에 확정 반영 완료 |
| 2 | `backdrop-filter` 폴백 검수 (구형 Firefox) | 스크림 알파 상향으로 대응 예정 |
| 3 | 지도 위 날씨×낮밤 오버레이 레이어의 알파값 | 구현 단계에서 시각 검수 후 확정(§3.1 배경 그라디언트 값 참고, 타일 위이므로 더 낮은 알파 필요할 수 있음) |
